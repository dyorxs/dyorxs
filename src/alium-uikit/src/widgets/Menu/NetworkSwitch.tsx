...
const StyledDropDown = styled.div<{ hasError: boolean }>`
  width: 232px;
  border: 1px solid ${(props) => (props.hasError ? 'transparent' : '#67c200')};
  ...
  :hover {
    background-color: ${(props) => (props.hasError ? '#ff4d00cf' : '#67c200;')};
    > * {
      color: white;
    }
    > svg * {
  ...
  > svg {
    fill: ${(props) => (props.hasError ? '#fff' : '#67c200;')};
    path {
      stroke: ${(props) => (props.hasError ? '#fff' : '#67c200;')};
    }
    position: absolute;
    right: 23px;
    top: 18px;
